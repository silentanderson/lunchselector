...
class App(customtkinter.CTk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 참여 인원을 위한 Checkbox 위젯
        checkbox_title = "Members"
        checkbox_texts = ["Thomas", "Kevin", "Henry", "Andy", "John", "David"]
        entrybox_title = "Recommended Menu"
        ...
        # 거리 입력을 위한 Slider 위젯
        self.dist_slider = CTkSlider(self, orientation="horizontal",
                                     from_=0, to=500, number_of_steps=10, hover=True, command=self.update_dist)
        self.dist_slider.grid(row=1, column=0, padx=10, pady=10,
                              sticky="ew", columnspan=2)

        # 거리 표시 라벨
        self.dist_label = CTkLabel(
            self, text='거리: 250 m', fg_color="gray30")
    ...
    def select_menu(self):
        ...
        if not menu_list:
            messagebox.showerror("Error", "Recommended menu is not typed yet.")
        else:
            # 랜덤으로 선택된 메뉴를 표시
            selected_menu = random.choice(menu_list)
            messagebox.showinfo(
                "Result", f"Members for lunch together are : {ppl_list}, Total number of people is {ppl_count} ! Menu for today's lunch is going to be {selected_menu} !")
        print(self.update_dist())

    def update_dist(self, *args):
        dist_val = int(self.dist_slider.get())
        app.dist_label.configure(text=f"거리: {dist_val} m")
